...
netstreamSWRP = netstreamSWRP or {}

local stored = netstreamSWRP.stored or {}
netstreamSWRP.stored = stored

local cache = netstreamSWRP.cache or {}
netstreamSWRP.cache = cache


-- A function to split data for a data stream.
function netstreamSWRP.Split(data)
	local index = 1
	local result = {}
	local buffer = {}
...
--]]
function netstreamSWRP.Hook(name, Callback)
	stored[name] = Callback
end


if (SERVER) then
	util.AddNetworkString("SWRPNetStreamDS")
	util.AddNetworkString("SWRPNetStreamHeavy")

	-- A function to start a net stream.
	function netstreamSWRP.Start(player, name, ...)
		local recipients = {}
	...
	function netstreamSWRP.Heavy(player, name, ...)
		local recipients = {}
		local bShouldSend = false
		...
		local encodedData = pon.encode({...})
		local split = netstreamSWRP.Split(encodedData)

		if (encodedData and #encodedData > 0 and bShouldSend) then
			for k, v in ipairs(split) do
				net.Start("SWRPNetStreamHeavy")
					net.WriteString(name)
					net.WriteUInt(#v, 32)
					net.WriteData(v, #v)
					net.WriteUInt(k, 8)
	...
	function netstreamSWRP.Listen(name, Callback)
		netstreamSWRP.Hook(name, function(player, data)
			local bShouldReply, reply = Callback(player, data)

			if (bShouldReply) then
				netstreamSWRP.Start(player, name, reply)
			end
		end)
	end

	net.Receive("SWRPNetStreamDS", function(length, player)
			...
			if (player.nsDataStreamName and player.nsDataStreamData) then
				...
				if (stored[player.nsDataStreamName]) then
					...
					if (bStatus) then
						...
						ErrorNoHalt("NetStreamSWRP: '"..NS_DS_NAME.."'\n"..value.."\n")
					end
				end
	...
	net.Receive("SWRPNetStreamHeavy", function(length, player)
		local NS_DS_NAME = net.ReadString()
		local NS_DS_LENGTH = net.ReadUInt(32)
		local NS_DS_DATA = net.ReadData(NS_DS_LENGTH)
	...
	-- A function to start a net stream.
	function netstreamSWRP.Start(name, ...)
		local encodedData = pon.encode({...})
	...
	-- A function to start a net stream.
	function netstreamSWRP.Heavy(name, ...)
		local dataTable = {...}
	...
	function netstreamSWRP.Request(name, data, Callback)
		netstreamSWRP.Hook(name, Callback)		
		netstreamSWRP.Start(name, data)
	end

	net.Receive("SWRPNetStreamDS", function(length)
		local NS_DS_NAME = net.ReadString()
		local NS_DS_LENGTH = net.ReadUInt(32)
		local NS_DS_DATA = net.ReadData(NS_DS_LENGTH)
	...
	net.Receive("SWRPNetStreamHeavy", function(length)
		local NS_DS_NAME = net.ReadString()
		local NS_DS_LENGTH = net.ReadUInt(32)
		local NS_DS_DATA = net.ReadData(NS_DS_LENGTH)
		local NS_DS_PIECE = net.ReadUInt(8)
			...
			if (NS_DS_PIECE < NS_DS_TOTAL) then
				...
				if (stored[NS_DS_NAME]) then
					...
					if (bStatus) then
						...
						ErrorNoHalt("NetStreamSWRP Heavy: '"..NS_DS_NAME.."'\n"..value.."\n")
					end
