...
local function pp_open_menu()
	...
	ClearMapProps.DoClick = function()
		...
		local ClearMapPropsFrame = vgui.Create( "DFrame" )
		ClearMapPropsFrame:SetText( " Clear map props in the DB " )
		ClearMapPropsFrame:SetSize( 400, 150)
		ClearMapPropsFrame:SetPos( 400, 300 )
		local CMPFT = vgui.Create("DLabel" , ClearMapPropsFrame)
		CMPFT:Dock(TOP)
		CMPFT:SetText("Are you sure you want clear map props in the db ? \nYou can't undo this action !")
		CMPFT:SetFont("pp_font")
		CMPFT:SetSize(300,50)
		ClearMapPropsFrame:MakePopup()
		local CMPFB1 = vgui.Create("DButton" , ClearMapPropsFrame)
		CMPFB1:Dock(TOP)
		CMPFB1:SetText("Yes!")
		CMPFB1:SetFont("pp_font")
		CMPFB1.DoClick = function()
			net.Start("pp_info_send")
			net.WriteTable({CMD = "DEL_MAP"})
			net.SendToServer()
			ClearMapPropsFrame:Remove()
		end
		
		local CMPFB2 = vgui.Create("DButton" , ClearMapPropsFrame)
		CMPFB2:Dock(TOP)
		CMPFB2:SetText("Cancel")
		CMPFB2:SetFont("pp_font")
		CMPFB2.DoClick = function()
			ClearMapPropsFrame:Remove()
		end
	...
	ClearAllProps.DoClick = function()
		local ClearAllPropsFrame = vgui.Create( "DFrame" )
		ClearAllPropsFrame:SetText( " Clear map props in the DB " )
		ClearAllPropsFrame:SetSize( 400, 150)
		ClearAllPropsFrame:SetPos( 400, 300 )
		local CAPFT = vgui.Create("DLabel" , ClearAllPropsFrame)
		CAPFT:Dock(TOP)
		CAPFT:SetText("Are you sure you want clear all props in the db ?\nYou can't undo this action !")
		CAPFT:SetFont("pp_font")
		CAPFT:SetSize(300,50)
		ClearAllPropsFrame:MakePopup()
		local CAPFB1 = vgui.Create("DButton" , ClearAllPropsFrame)
		CAPFB1:Dock(TOP)
		CAPFB1:SetText("Yes!")
		CAPFB1:SetFont("pp_font")
		CAPFB1.DoClick = function()
			net.Start("pp_info_send")
			net.WriteTable({CMD = "DEL_ALL"})
			net.SendToServer()
			ClearAllPropsFrame:Remove()
		end
		
		local CAPFB2 = vgui.Create("DButton" , ClearAllPropsFrame)
		CAPFB2:Dock(TOP)
		CAPFB2:SetText("Cancel")
		CAPFB2:SetFont("pp_font")
		CAPFB2.DoClick = function()
			ClearAllPropsFrame:Remove()
		end
